...
% ========== Toolbox paths ==========
tb = mini2p_toolbox_paths();
EXTRACT_path   = tb.EXTRACT;
NoRMCorre_path = tb.NoRMCorre;
ActSort_path   = tb.ActSort;
...
P.mc_max_shift = 15;
P.mc_us_fac = 50;
P.use_zscore_before_extract = false;   % keep raw F so (F-F0)/F0 is real dF/F
P.extract_bin_time = 4;
P.trace_on_unbinned = true;            % find on binned, trace at full rate
P.extract_mode = 'final';
P.extract_preset = 'low_snr';          % measured SNR is 0.75 unbinned
...
P.avg_cell_radius = 8;
% Hand-curated footprint set. With extract_mode 'curated' the cell-finding
% pass is skipped entirely and these footprints are taken as config.S_init on
% the UNBINNED movie, so EXTRACT's own solver demixes them. That is what makes
% a split pair usable: a footprint-weighted average of two adjacent somata
% cross-contaminates them, the T-step does not. Requires trace_on_unbinned.
P.curated_S_file = '';
P.curated_max_iter = 1;            % T update only; 2 let the S-step
                                   % relocate footprints by 27-89 px
P.skip_morph_filter = false;       % the curation already made these calls
P.skip_video_01 = true;            % skip 01_check_input (speed)
P.skip_video_03 = true;            % skip 03_check_after_denoising (speed)
P.check_video_start_frame = 50;    % skip first N frames in check videos (avoids black MC frames)
...
% Every later save into base_filename uses '-append', which needs the file to
% already exist. It is normally created by the load step, but skip_steps_1_2
% bypasses that, so the curated pass reached the first append with no file and
% died with "No such file or directory".
if ~isfile(fullfile(output_folder, base_filename))
    results_created = char(datetime('now'));
    save(fullfile(output_folder, base_filename), 'results_created', '-v7.3');
end
...
start_fr = max(1, P.check_video_start_frame);
write_check_video = @(mov, fname, nf) write_uint_video(mov(:,:,min(start_fr,size(mov,3)):end), fullfile(output_folder, fname), nf, P.check_video_fps);
...
if P.skip_steps_1_2
    ...
    % Re-generate MC check video from saved denoised data (skipping black frames)
    if P.n_frames_check > 0
        write_check_video(denoised_data, '02_check_after_motion_correction.avi', P.n_frames_check);
        disp('  Re-wrote 02_check_after_motion_correction.avi (skip black frames)');
    end
else
...
% Keep the UNBINNED movie when traces are wanted at the full frame rate.
% Binning is what makes the cells findable here (measured SNR 0.75 unbinned,
% 1.50 at bin 4) but at bin 4 the rate falls to 1.15 Hz, which leaves only
% two samples inside the F0 window [-3,-1] s and two inside the response
% window [1,3] s that downstream_step2 uses. Two samples cannot define a
% baseline. So: find the cells on the binned movie, then reuse those spatial
% footprints on the unbinned movie to get traces at 4.605 Hz.
M_unbinned = [];
if P.extract_bin_time > 1
    if P.trace_on_unbinned
        M_unbinned = M_for_extract;
    end
    ...
    clear M_binned;
elseif P.trace_on_unbinned
    disp('  trace_on_unbinned requested but bin factor is 1 - nothing to do.');
end
...
elseif strcmpi(P.extract_preset, 'low_snr')
    spatial_highpass_cutoff = 8;
else
...
elseif strcmpi(P.extract_preset, 'low_snr')
    % Tuned for: low-SNR FOV with ~20 real neurons that are LARGE.
    % Strategy: find candidates with moderate sensitivity, merge duplicates
    % aggressively, and use a larger numpix_threshold + post-hoc area filter
    % to reject small noise blobs that are clearly not neurons.
    config.cellfind_max_steps = 2000;
    config.cellfind_min_snr   = 0.8;
    config.cellfind_numpix_threshold = 25;  % require seed >= 25 px (rejects tiny noise seeds)
    config.init_with_gaussian = true;
    config.thresholds.eccent_thresh = 7;
    config.smooth_S = true;
    config.thresholds.spatial_corrupt_thresh = 1.3;
    config.thresholds.size_upper_limit = 10;
    config.thresholds.size_lower_limit = 0.25;  % reject ROIs below 25% of expected radius
    config.thresholds.T_min_snr = 3.5;
    config.thresholds.low_ST_index_thresh = -0.05;
    config.thresholds.low_ST_corr_thresh  = 0;
    config.thresholds.S_dup_corr_thresh = 0.65;
    config.thresholds.T_dup_corr_thresh = 0.65;
    config.max_iter = 8;
    config.num_partitions_x = 1;
    config.num_partitions_y = 1;
else
...
switch lower(P.extract_mode)
    ...
    case 'step2'
        ...
        return;
    case 'curated'
        % Traces for a hand-curated footprint set, straight onto the unbinned
        % movie. No cell finding, and every quality gate is off: the keep or
        % discard decisions were already made by eye, so re-applying the
        % automatic filters here would silently drop curated cells and break
        % the correspondence with curated_labels.
        if isempty(M_unbinned)
            error(['extract_mode ''curated'' needs trace_on_unbinned = ' ...
                   'true so the unbinned movie is available.']);
        end
        if ~isfile(P.curated_S_file)
            error('curated_S_file not found: %s', P.curated_S_file);
        end
        C = load(P.curated_S_file);
        n_cur = size(C.S, 2);
        if size(C.S, 1) ~= size(M_unbinned, 1) * size(M_unbinned, 2)
            error(['curated S has %d rows but the FOV is %d x %d = %d ' ...
                   'pixels'], size(C.S, 1), size(M_unbinned, 1), ...
                  size(M_unbinned, 2), ...
                  size(M_unbinned, 1) * size(M_unbinned, 2));
        end
        disp(['----- Curated pass: ', num2str(n_cur), ' footprints onto ', ...
              num2str(size(M_unbinned, 3)), ' unbinned frames -----']);
        M_full = spatial_bandpass(M_unbinned, avg_cell_radius, ...
                                  spatial_highpass_cutoff, inf, 0);
        M_unbinned = [];            % also skips the second pass below
        cfgc = config;
        cfgc.S_init = double(C.S);
        cfgc.cellfind_max_steps = 0;
        cfgc.max_iter = P.curated_max_iter;
        cfgc.remove_duplicate_cells = 0;
        % Every gate in get_bad_components (a local function of
        % remove_redundant) has to be opened, not most of them. Setting a
        % subset the first time still lost 3 to 7 cells per plane, because
        % temporal_corrupt_thresh was left at its 0.7 default. The full list
        % is get_defaults.m lines 7-19:
        cfgc.thresholds.T_min_snr = -inf;          % 0 still fires on T<=0
        cfgc.thresholds.size_lower_limit = -inf;
        cfgc.thresholds.size_upper_limit = inf;
        cfgc.thresholds.temporal_corrupt_thresh = inf;
        cfgc.thresholds.spatial_corrupt_thresh = inf;
        cfgc.thresholds.eccent_thresh = inf;
        % exactly -1 makes remove_redundant skip find_spurious_cells
        % altogether and set the ST metrics to 1, which disables that gate
        % and is far cheaper than -inf
        cfgc.thresholds.low_ST_index_thresh = -1;
        cfgc.thresholds.low_ST_corr_thresh = -inf;
        % find_conncomp thresholds on C > threshold, and a correlation is
        % never above 1, so 1 disables both merge paths
        cfgc.thresholds.S_dup_corr_thresh = 1;
        cfgc.thresholds.T_dup_corr_thresh = 1;
        % Joint least squares is computed ALWAYS, not only as a fallback.
        % One unconditional clause in remove_redundant cannot be opened by
        % any threshold (is_S_tiny includes S_smooth_area_1 == 0), and it
        % drops exactly the footprints curation added: 3 of 16 in open-field
        % plane B and 2 of 16 in pain plane B. Using EXTRACT on the planes
        % where it keeps everything and least squares on the others would
        % mean two different trace methods across planes, which is not
        % comparable. Computing both lets the difference be measured instead
        % of assumed - see compare_curated_traces.py.
        Sc = double(C.S);
        Sc = Sc ./ max(sum(Sc, 1), eps);
        Mf = reshape(single(M_full), [], size(M_full, 3));
        T_ls = ((Sc' * Sc) \ (Sc' * double(Mf)))';      % (frames x cells)
        clear Mf;
        S_cur3 = reshape(double(C.S), size(M_full, 1), size(M_full, 2), n_cur);
        % A footprint must be one compact blob. If the file was written with
        % numpy's default C-order ravel, MATLAB's column-major reshape
        % scatters each soma into thin stripes spanning the whole FOV - one
        % 212 px cell became 17 pieces of 16 px across all 440 rows, and
        % every trace solved against it was wrong while nothing errored. So
        % this is checked rather than trusted.
        for ii = 1:n_cur
            im = S_cur3(:, :, ii);
            bw = im > 0.3 * max(im(:));
            cc = bwconncomp(bw, 8);
            np_ = cellfun(@numel, cc.PixelIdxList);
            big = sum(np_ >= 20);
            [rr, ~] = find(bw);
            if big == 0 || (cc.NumObjects > 6 && big <= 1) || ...
                    (max(rr) - min(rr)) > 0.8 * size(S_cur3, 1)
                error(['Curated footprint %d is %d fragments spanning %d ' ...
                       'of %d rows, not a cell. curated_S.mat was almost ' ...
                       'certainly written in C order; it must be ' ...
                       'Fortran order so reshape() is correct.'], ...
                      ii, cc.NumObjects, max(rr) - min(rr) + 1, ...
                      size(S_cur3, 1));
            end
        end
        % plot_output_cellmap reads output.info.summary as an array of
        % per-partition EXTRACT diagnostics. An empty struct array is the
        % honest value when EXTRACT did not produce the traces: length() is
        % 0, so the bad-cell loop finds nothing to plot. Inventing
        % classification data here would be fabricating EXTRACT internals.
        info_cur = struct('summary_image', mean(M_full, 3), ...
                          'max_image', max(M_full, [], 3), ...
                          'summary', struct('classification', {}, ...
                                            'S_change', {}));

        out_ex = extractor(M_full, cfgc);
        n_back = size(out_ex.temporal_weights, 2);
        % Measure how far EXTRACT's S-step moved each footprint. With
        % max_iter = 2 it moved them a median of 27 px (open field A) and
        % 89 px (pain A), with several collapsing onto the same location, so
        % its traces belonged to different places than the curated cells and
        % correlated with the least-squares ones at r ~ 0. The curated
        % geometry is the truth here, so it is always what gets saved; any
        % refined footprints are kept separately and only used if they stayed
        % put.
        ex_shift = NaN;
        if n_back == n_cur
            ex_shift = 0;
            for ii = 1:n_cur
                a = out_ex.spatial_weights(:, :, ii);
                b = S_cur3(:, :, ii);
                [ya, xa] = find(a > 0.3 * max(a(:)));
                [yb, xb] = find(b > 0.3 * max(b(:)));
                if isempty(ya) || isempty(yb), ex_shift = inf; break; end
                ex_shift = max(ex_shift, hypot(mean(ya) - mean(yb), ...
                                               mean(xa) - mean(xb)));
            end
        end
        output = struct();
        output.spatial_weights = S_cur3;          % curated geometry, always
        output.spatial_weights_curated = S_cur3;
        output.temporal_weights = T_ls;           % uniform across planes
        output.temporal_weights_ls = T_ls;
        output.trace_method = 'joint least squares';
        output.config = cfgc;
        if n_back == n_cur && isfinite(ex_shift) && ex_shift <= avg_cell_radius
            output.info = out_ex.info;
            output.temporal_weights_extract = out_ex.temporal_weights;
            output.spatial_weights_extract = out_ex.spatial_weights;
            output.extract_max_shift_px = ex_shift;
            disp(['  EXTRACT T-step traces also saved (footprints moved at ' ...
                  'most ', num2str(ex_shift, '%.1f'), ' px)']);
        else
            output.info = info_cur;
            if n_back ~= n_cur
                warning(['Curated EXTRACT pass returned %d of %d cells; ' ...
                         'its traces are not saved.'], n_back, n_cur);
            else
                warning(['EXTRACT S-step moved a footprint by %.1f px ' ...
                         '(> radius %d), so its traces no longer belong to ' ...
                         'the curated cells and are not saved.'], ...
                        ex_shift, avg_cell_radius);
            end
        end
        clear out_ex;
        disp(['  traces: least squares on the curated footprints, ', ...
              num2str(n_cur), ' cells']);
        clear M_full M_input S_cur3;
        output.traces_are_full_rate = true;
        output.trace_frame_rate_hz = P.ca_frame_rate_hz;
        if isfield(C, 'labels'), output.curated_labels = C.labels; end
        disp(['  ', num2str(n_back), ' traces at ', ...
              num2str(P.ca_frame_rate_hz), ' Hz (', ...
              num2str(size(output.temporal_weights, 1)), ' frames)']);
    otherwise
        output = extractor(M_input, config);
end
clear M_input;

%% ----- Second pass: traces at the full frame rate -----
% The footprints found on the binned movie are reused on the unbinned one via
% config.S_init, so EXTRACT's own trace solver produces the traces rather
% than a hand-rolled least-squares step. Cell identity, count and order are
% inherited from the binned pass; only the temporal weights change.
if P.trace_on_unbinned && ~isempty(M_unbinned)
    n_found = size(output.spatial_weights, 3);
    if n_found == 0
        warning('No cells from the binned pass - skipping the full-rate pass.');
    else
        disp(['----- Full-rate trace pass: ', num2str(n_found), ...
              ' footprints onto ', num2str(size(M_unbinned, 3)), ' frames -----']);
        S_keep = reshape(output.spatial_weights, [], n_found);
        M_full = spatial_bandpass(M_unbinned, avg_cell_radius, ...
                                  spatial_highpass_cutoff, inf, 0);
        clear M_unbinned;
        ...
        cfg2 = config;
        cfg2.S_init = S_keep;
        cfg2.cellfind_max_steps = 0;   % do not look for new cells
        cfg2.max_iter = 1;             % one alternating update for T
        cfg2.remove_duplicate_cells = 0;
        % keep every input footprint: the quality gates already ran on the
        % binned pass, and re-applying them here would silently drop cells
        % and break the correspondence with output.spatial_weights
        cfg2.thresholds.T_min_snr = 0;
        cfg2.thresholds.size_lower_limit = 0;
        cfg2.thresholds.size_upper_limit = inf;
        cfg2.thresholds.eccent_thresh = inf;
        cfg2.thresholds.spatial_corrupt_thresh = inf;
        cfg2.thresholds.low_ST_index_thresh = -inf;
        cfg2.thresholds.low_ST_corr_thresh = -inf;
        cfg2.thresholds.S_dup_corr_thresh = 1;
        cfg2.thresholds.T_dup_corr_thresh = 1;

        ok = false;
        try
            out_full = extractor(M_full, cfg2);
            n_back = size(out_full.temporal_weights, 2);
            if n_back == n_found
                output.temporal_weights_binned = output.temporal_weights;
                output.temporal_weights = out_full.temporal_weights;
                output.spatial_weights_fullpass = out_full.spatial_weights;
                output.trace_frame_rate_hz = P.ca_frame_rate_hz;
                output.binned_frame_rate_hz = ...
                    P.ca_frame_rate_hz / P.extract_bin_time;
                ok = true;
                disp(['  traces replaced at ', ...
                      num2str(P.ca_frame_rate_hz), ' Hz (', ...
                      num2str(size(output.temporal_weights, 1)), ' frames)']);
            else
                warning(['Full-rate pass returned %d cells for %d ' ...
                         'footprints; cell-to-trace correspondence is not ' ...
                         'safe, so the binned traces are kept.'], ...
                        n_back, n_found);
            end
        catch ME
            warning('Full-rate pass failed (%s). Binned traces kept.', ...
                    ME.message);
        end
        output.traces_are_full_rate = ok;
        clear M_full out_full S_keep;
    end
elseif ~isfield(output, 'traces_are_full_rate')
    % the curated pass already set this to true and cleared M_unbinned, so
    % it must not be overwritten here
    output.traces_are_full_rate = false;
end
...
if strcmpi(P.extract_preset, 'low_snr')
    % Real neurons are large (~radius 8 → area ~200 px).
    % Remaining noise ROIs are 60-120 px. Raise floor to reject them.
    min_area_px = round(pi * (avg_cell_radius * 0.85)^2);  % ~145 px
...
if P.skip_morph_filter
    n_would = sum(~keep);
    keep(:) = true;
    disp(['Morphology filter skipped (curated set); it would have ' ...
          'dropped ', num2str(n_would), ' of ', num2str(numel(keep)), ...
          ' cells.']);
end
...
if P.n_frames_check > 0 && n_final > 0
    overlay_start = min(start_fr, size(denoised_data, 3));
    write_check_video_extract_overlay(denoised_data(:,:,overlay_start:end), output.spatial_weights, ...
...
if exist('plot_output_cellmap', 'file') == 2
    % Cosmetic figure that reaches into EXTRACT's internal diagnostics, so it
    % must not be able to kill a run that has already produced the traces.
    try
        fig5 = figure;
        plot_output_cellmap(output, [], [], 'clim_scale', [0.2, 0.999]);
        saveas(fig5, fullfile(output_folder, 'extract_cellmap.png'));
    catch ME
        warning('extract_cellmap.png skipped (%s).', ME.message);
