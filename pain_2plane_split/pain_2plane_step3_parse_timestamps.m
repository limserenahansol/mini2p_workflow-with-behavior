...
% ---- Behavioral camera 1 (optional; open-field sessions may have only cam2) ----
behav1_timestamps = datetime.empty(0, 1);
behav1_seconds = [];
behav1_tdms = find_tdms(fullfile(session_dir, 'MiceVideo1'), '*reference.tdms', false);
if isempty(behav1_tdms)
    disp('  Behavior cam1 TDMS: none (MiceVideo1 missing)');
else
...
% ---- Behavioral camera 2 ----
behav2_timestamps = datetime.empty(0, 1);
behav2_seconds = [];
behav2_tdms = find_tdms(fullfile(session_dir, 'MiceVideo2'), '*reference.tdms', false);
if isempty(behav2_tdms)
    disp('  Behavior cam2 TDMS: none (MiceVideo2 missing)');
else
...
if ~isempty(behav1_seconds)
    behav_frame_rate_hz = 1 / median(diff(behav1_seconds));
elseif ~isempty(behav2_seconds)
    behav_frame_rate_hz = 1 / median(diff(behav2_seconds));
else
    behav_frame_rate_hz = NaN;
    warning('No behavioral camera timestamps found.');
end
if isfinite(behav_frame_rate_hz)
...
function fpath = find_tdms(folder, pattern, required)
